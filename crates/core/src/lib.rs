...
impl<E: Embedder> Database<E> {
    ...
    pub fn open(embedder: E, dir: impl AsRef<Path>) -> Result<Self> {
        ...
        if model_id != embedder.model_id() {
            return Err(Error::Config(format!(
                "store was built with model '{}' but embedder is '{}'",
                model_id,
                embedder.model_id()
            )));
        }
    ...
    /// Embed `text` and add it under `id`. Errors if `id` already exists.
    pub fn add(&mut self, id: &str, text: &str) -> Result<()> {
        let v = self.embedder.embed(text)?;
        self.index.add(id, v)
    }

    /// Add a precomputed vector under `id`.
    pub fn add_vector(&mut self, id: &str, vector: Vec<f32>) -> Result<()> {
        self.index.add(id, vector)
    }

    /// Re-embed `text` and replace the vector for an existing `id`.
...
mod tests {
    ...
    fn add_query_roundtrip() {
        ...
        db.add("dog", "a lazy sleepy dog rests").unwrap();
    ...
    fn k_zero_and_k_larger_than_len() {
        let mut db = db();
        db.add("a", "alpha").unwrap();
        db.add("b", "beta").unwrap();
        assert!(db.query("alpha", 0).unwrap().is_empty());
        assert_eq!(db.query("alpha", 100).unwrap().len(), 2);
    }
